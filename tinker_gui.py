...
space.grid(row=5, column=0, padx=5, pady=20)


# ---- Select Colleague Drop Down Menu

existing_colleague_label = Label(left_frame, text="Select Existing Colleague")
existing_colleague_label.grid(row=6, column=0, padx=5, pady=5, columnspan=2)

clicked = StringVar()
colleague_drop = OptionMenu(left_frame, clicked, colleagues)
colleague_drop.grid(row=7, column=0, padx=10, pady=5, columnspan=2)

# ------- Right FRAME ----------

#====Images====#
image = Label(root, bitmap="")
image.grid(row=0, column=1, padx=5, pady=5, columnspan=2)
